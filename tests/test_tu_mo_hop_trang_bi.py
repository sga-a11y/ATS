...
class TestGoiDonateTrangBi(unittest.TestCase):
    def test_dung_sub_35_va_KHONG_co_truong_tien(self):
        t = _than("donate_legion_equip")
        self.assertIn('self.send(0x27, b"\\x35\\x00" + bytes(ds))', t)
        self.assertNotIn("struct.pack", t, "goi nay KHONG co truong tien nhu donate nguyen lieu")
...
class TestGomNhieuSlotMotLenh(unittest.TestCase):
    ...
    def test_trang_bi_gom_mot_lenh(self):
        t = _than("donate_legion_equip")
        self.assertIn(r'self.send(0x27, b"\x35\x00" + bytes(ds))', t)
