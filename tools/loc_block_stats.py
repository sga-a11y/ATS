# -*- coding: utf-8 -*-
"""Loc `train_block_stats.json`: BO cac pattern chiem duoi 1% so tran cua diem do.
...
def main(argv=None):
    ...
    for mkey, mval in (data.get("maps") or {}).items():
        for skey, spot in (mval.get("spots") or {}).items():
            ...
            for k, n, ti in sorted(bo, key=lambda r: -r[1]):
                dong.append("map %-6s %-12s  %-14s %5d tran (%.2f%%)" % (mkey, skey, k, n, ti * 100))
    ...
    print("-" * 60)
    print("BO %d pattern / %d tran (nguong %.2f%%)" % (tong_pat, tong_tran, a.nguong * 100))
